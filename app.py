# AutoJournal v4 — Cipher Ghost Personal OS
...
<style>
  ...
  .stExpander { background: white !important; border-radius: 12px !important; border: 1px solid #ddd !important; margin-bottom: 0.7rem !important; box-shadow: 0 2px 8px rgba(0,0,0,0.06) !important; }
  .stExpander summary { padding: 0.8rem 1rem !important; }
  ...
  details summary { color: #1a1a2e !important; font-weight: 700 !important; }
  details { background: white !important; border-radius: 12px !important; }
</style>
    ...
    try:
        ...
        for e in entries:
            ...
            if not is_empty_entry(extracted):
                ...
                valid.append(e)
        if not valid:
            st.markdown('<div class="card"><p style="color:#999;font-size:0.9rem;text-align:center;margin:0;font-family:Montserrat;">No entries yet.<br>Submit your first entry above.</p></div>', unsafe_allow_html=True)
            return
        ...
        for e in valid:
            ...
            mood_colors = {
                "excellent": "#27ae60", "good": "#2ecc71", "focused": "#4834d4",
                "grateful": "#e67e22", "tired": "#7f8c8d", "stressed": "#e74c3c",
                "neutral": "#95a5a6", "low": "#c0392b"
            }
            mood_color = mood_colors.get(str(mood).lower(), "#4834d4")

            st.markdown(f"""
            <div style="background:white;border-radius:14px;border:1px solid #e0e0e0;
                 box-shadow:0 3px 12px rgba(0,0,0,0.08);margin-bottom:0.5rem;overflow:hidden;">
              <div style="background:linear-gradient(135deg,#1a1a2e,#0f3460);
                   padding:0.9rem 1.1rem;display:flex;justify-content:space-between;align-items:center;">
                <div>
                  <div style="font-family:'Playfair Display',serif;font-size:1rem;
                       font-weight:700;color:#d4af37;letter-spacing:0.03em;">
                    📅 {display_date}
                  </div>
                  <div style="font-family:'Oswald',sans-serif;font-size:0.65rem;
                       color:rgba(255,255,255,0.6);letter-spacing:0.15em;margin-top:0.2rem;">
                    CIPHER GHOST LOG
                  </div>
                </div>
                <div style="background:{mood_color};color:white;border-radius:20px;
                     padding:0.2rem 0.9rem;font-family:'Oswald',sans-serif;
